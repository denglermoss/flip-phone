...
**No open revisit items remain.** All four scheduled revisits (modem, codec, display, USB HS/ULPI) are resolved. The revisit prompt files have been **archived** to `docs/archive/revisit-prompts/` (see `docs/archive/revisit-prompts/README.md` for the archive index). The prompts are retained for historical reference only — do not re-run them.
