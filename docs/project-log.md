...
- **Remaining schematic-time check**: Verify SIM7600 PCM pin I/O voltage in the Hardware Design Manual. Expected 1.8V — if so, PCM lines connect directly to MAX9880A with no level shifting. If higher, add a level shifter on the PCM lines (in addition to the already-planned shifter on the MCU I2S lines).
- **Impact**: The MAX9880A fallback (MCU bridge with NAU8822) remains documented but is now unlikely to be needed. The codec is fully de-risked for PCB commit.
