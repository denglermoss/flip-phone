...
- **Pre-PCB verification — COMPLETE 2026-06-29**: (1) **PCM short-frame sync: CONFIRMED COMPATIBLE** — MAX9880A TDM short-sync mode (TDM=1, FSW=0, MAS=0 slave) matches SIM7600's PCM master short-frame sync; TDM clock range 128–2048 kHz encompasses SIM7600's 2048 kHz BCLK; 16-bit voiceband on primary port. (2) **Stock: AVAILABLE** — Mouser has 2,250 in stock (700-MAX9880AETM+T, $2.23 qty 1); DigiKey temporarily out (normal for ADI/Maxim); part is active, not discontinued. (3) **1.8V supply + level shifting**: documented — 1.8V LDO from 3.3V rail, unidirectional level shifter for MCU I2S lines. Schematic-time check: verify SIM7600 PCM pin I/O voltage (expected 1.8V — if so, PCM lines connect directly to MAX9880A). Fallback if a blocking issue emerges later: MCU bridge with NAU8822 (see research-notes.md Codec Selection Finding 3).
