...
### PCB Design
- Single-board design (candybar form factor, LOCKED 2026-08-16). No multi-board split — display mounts directly via ZIF connector. Flip/clamshell deferred to v2.
...
- **Board size target (MPCIe variant, 2026-07-22, updated 2026-08-16)**: **Single board < 2×3 inches (~50×76mm, ~3800mm²) as a target — but candybar form factor allows a larger board if needed.** First-pass estimate was ~100×65mm (conservative, most parts on top); revised to ~55×78mm with aggressive 2-sided placement (MCU + most ICs on bottom under keypad zone or under display; MPCIe + keypad stacked on top sharing the 54mm width). Candybar benefit: components can go under the display area, no hinge placement constraint. Deferred to KiCad layout for real numbers — not recalculated yet. MPCIe modem (54×32mm keepout) is the largest single block and sets the ~54mm minimum dimension. ~~Display daughterboard ~55×42mm~~ **DROPPED 2026-08-16** (candybar — no daughterboard). See project-log.md 2026-07-22 Board Size Estimate + 2026-08-16 Form Factor Pivot.
...
- **MPCIe form factor (primary 2026-07-22, PCM confirmed 2026-07-22)**: SIM7600NA-H-PCIE (S2-109KS-Z30G9) from Techship ~$50/ea — Mini PCIe socketed card. Same B71/B66/VoLTE/GNSS/PCM specs as the LGA NA-H. **PCM CONFIRMED** from SIM7600 Series PCIE Hardware Design V1.03 §2.2 (p14) and §3.10.1 (p29): pins 45 (PCM_CLK), 47 (PCM_OUT), 49 (PCM_IN), 51 (PCM_SYNC) — master mode, short sync, 16-bit, 2048kHz, 1.8V. Exact match to ALC5651 PCM Mode A. PCIE variant = no onboard codec = PCM active; PCIEA variant = onboard NAU8810 = PCM NC (analog audio instead). Part S2-109KS-Z30G9 ends in PCIE (not PCIEA). **All MPCIe socket options are SMD** (0.8mm pitch, right-angle, surface mount — NOT through-hole as previously stated). Socket reflows with the rest of the board — no iron work needed. Socket options: Amphenol G633A0520022U (LCSC C357792, 1589 stock, ~5mm height), SOFNG PCIE-52P40H (LCSC C444926, 3039 stock, 4mm height), Hanbo PCI-E-H52-52P (LCSC C3039346), **JLC basic C9900027618** (5.2mm height, no extended fee if going JLC PCBA). Dimensions: 50.8×31×5.35mm card (vs LGA 30×30×1.5mm — adds ~21mm length, ~4mm height). **Additional findings from V1.03 manual**: (1) SIM socket NOT onboard (§3.7 says "contact local sales" — Techship spec lists it as optional `o`, stock card does NOT have it; both variants need PCB SIM socket wired to USIM pins); (2) UART is 1.8V (§3.8, same as bare LGA — level shifter needed either way); (3) Pin mapping differs from LGA: PERST# (pin 22, reset), WAKE# (pin 1, wake host), W_DISABLE# (pin 20, RF disable), LED_WWAN# (pin 42, network status) — **no PWRKEY pin** (TBD how MPCIe module powers on/off); (4) I2C at 1.8V (§3.9, pulled up in-module); (5) USB 2.0 HS on pins 36/38 (same as LGA, VBUS internal to VBAT). **Approach: MPCIe is the primary form factor for the schematic + PCB layout. LGA (bare module) is the fallback — pivot only if MPCIe proves difficult during layout.** See project-log.md 2026-07-22 Schematic Approach + MPCIe Primary.
- **DIY assembly approach (under evaluation 2026-07-22)**: User considering full DIY assembly (no JLC PCBA) to save cost and as a learning goal. Only 1 board needed (no MOQ 2 without JLC PCBA). Plan: 1 modem + 2-3 board qty of all other parts (spares for DIY mistakes). Tools needed: reflow oven (T-962 ~$150) + preheater ($50-100) + stencil ($20-40, with modem area cut out if LGA) + solder paste ($15-30) + USB microscope ($30-50) + hot-air station ($50-100). 2-sided board: top (heavy: modem/MCU/codec/connectors) + bottom (light: ~20 keypad switches + passives). DIY sequence: stencil+populate+reflow bottom → flip → stencil+populate+reflow top. With MPCIe as the primary form factor, the modem socket is SMD (reflows with the rest of the board — no separate iron step) — remaining hard parts: ALC5651 QFN-40, TPS63021 VSON-14, FPC connectors 0.5mm pitch. See project-log.md 2026-07-22 assembly evaluation.
...
- Battery capacity vs form factor tradeoff — candybar / single-board form factor locked 2026-08-16. Final enclosure volume TBD (Phase 7 mechanical design).
...
### Form Factor
- **Candybar / single-board form factor LOCKED 2026-08-16.** One PCB with all components — display mounts directly via ZIF connector (J7, 12-pin 0.5mm). No hinge flex, no daughterboard, no outer display. See project-log.md 2026-08-16 Form Factor Pivot. (Supersedes 2026-07-19 flip/clamshell decision, which is deferred to v2.)
- **Board architecture** (single board):
  - MCU, modem, codec, keypad, battery, power (charger + buck-boost + LDO + fuel gauge), USB-C, SIM, SD, cellular/GNSS antennas, microphone, loudspeaker, level shifter, ESD protection, crystals, passives, **main display panel** (HS20HS072RX, 2.0" ST7789T3 via J7 ZIF), **earpiece speaker**, **backlight FET + resistors**.
  - Display panel is 3.3V-only TFT (no boost converter). **Display panel purchased separately from PCB (LCSC) and assembled by user** — ZIF connector is JLC-assembled on PCB; panel plugs in post-assembly. This decouples display sourcing from PCB fab.
  - ~~Outer display (ER-TFT1.14-2)~~ **DROPPED 2026-08-16** — only makes sense on a flip lid.
  - ~~Hinge flex cable~~ **DROPPED 2026-08-16** — no hinge, no daughterboard.
- **Camera**: Out of scope for rev1. Will go on the single board when added — DCMI parallel bus (11 pins + 1 power) stays on the board, no flex signals. The 41 spare GPIO on LQFP-144 already accounts for DCMI in the GPIO budget (now 43 spare with PG9/PG10 freed from outer display).
- ~~**Hinge mechanism**~~ **DROPPED 2026-08-16** (candybar — no hinge). ~~Deferred to Phase 7. Options: salvage from existing flip phone, custom 3D printed, off-the-shelf metal hinge, or CNC machined.~~
- **Enclosure design**: Deferred to Phase 7. 3D model in CAD, fit-check with single PCB. Single-shell enclosure (no hinge, no two-piece lid/base).
...
- **Major cost categories**:
  ...
  - ~~Flex cable / FPC: $5-20 (only if multi-board form factor chosen later)~~ **N/A 2026-08-16** (candybar — no flex cable)
