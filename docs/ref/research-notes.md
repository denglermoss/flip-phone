...
| Modem USB (D+/D-) | 2 | SIM7600 USB 2.0 HS port routed to USB2512 hub downstream 1 (rev1 hard requirement — 2026-08-27); hub presents modem + MCU over single USB-C |
...
- [x] **RESOLVED**: Which ESP32 variants have USB OTG? (ESP32-S2, ESP32-S3 have native USB; original ESP32 does not) — Moot; STM32H743 selected.
...
3. **1.8V supply — RESOLVED 2026-07-22**: ALC5651 operates on 1.8V analog (AVDD/DACREF/CPVDD, 1.71–1.9V), 1.2V digital core (internal LDO from DBVDD — no external 1.2V needed), 3.3V MICVDD for mic bias. **DBVDD is a SINGLE shared pin (pin 39)** — both I2S-1 and I2S-2 share the same voltage domain (verified from datasheet §6.4 Table 4, page 10). **Decision: DBVDD = 1.8V** (from U9 TPS7A0218). I2S-1 (PCM/modem) is direct 1.8V↔1.8V (no shifter). I2S-2 (MCU) goes through SN74AXC4T774 level shifter (U12, 3.3V↔1.8V). I2C bus uses 1.8V pullups (STM32H7 FT pins + MAX17048 VIH=1.4V min both work at 1.8V). TXB0108 is NOT recommended for I2S (TI SCEA135 — weak drive, edge-rate distortion). Power consumption: ≤5.5mW silence, ≤13mW playback, <9mW record (~3-7mA at 1.8V) — well under 50mA, but U9 retained (MPCIe doesn't expose VDD_1V8, and coupling codec power to modem state is undesirable).
...
4. **Route the SIM7600's USB D+/D- to the USB2512 hub** on rev1 (ecosystem is a hard rev1 requirement as of 2026-08-27 — no longer deferred to a future respin). The internal USB 2.0 hub (Topology A) presents both the modem (RNDIS, 480 Mbps) and the MCU (MSC) to the host over a single USB-C cable — simultaneous LTE + file access, no MCU USB bottleneck, no ULPI. See project-log.md 2026-08-27 USB Hub on Rev1.
5. **Update the "USB mode" resolved question**: USB FS for the MCU is the committed path; USB HS via ULPI is dropped (not needed — modem handles tethering at HS directly). The phone remains a USB **device** in all ecosystem scenarios; the modem is also a USB device, and the internal hub (on rev1) presents both to the host.
