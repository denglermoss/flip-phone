...
7. **MAX9880A codec availability/compatibility** (pre-PCB risk — **RESOLVED 2026-06-29**): The selected dual-port codec (MAX9880A) has been verified: (1) PCM short-frame sync is confirmed compatible — the MAX9880A's TDM short-sync slave mode (TDM=1, FSW=0, MAS=0) matches the SIM7600's PCM master short-frame sync output (2048 kHz BCLK, 16-bit); (2) stock is available at Mouser (2,250 units, $2.23 qty 1). The fallback (MCU bridge with NAU8822) remains documented but is now unlikely to be needed. One schematic-time check remains: verify SIM7600 PCM pin I/O voltage (expected 1.8V). The Waveshare HAT prototyping (NAU8810) validates the SIM7600 PCM voice output independent of this risk.
