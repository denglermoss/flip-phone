...
7. **MAX9880A codec availability/compatibility** (pre-PCB risk — **FULLY RESOLVED 2026-06-30**): The selected dual-port codec (MAX9880A) has been verified on all three pre-PCB items: (1) PCM short-frame sync is confirmed compatible — the MAX9880A's TDM short-sync slave mode (TDM=1, FSW=0, MAS=0) matches the SIM7600's PCM master short-frame sync output (2048 kHz BCLK, 16-bit); (2) stock is available at Mouser (2,250 units, $2.23 qty 1); (3) SIM7600 PCM pin voltage is 1.8V — confirmed 2026-06-30 from the Hardware Design Manual V1.03 (Table 32: 1.8V Digital I/O characteristics; §3.6.2 reference design wires PCM directly to a codec powered by VDD_1V8 with no level shifter). PCM lines connect directly to the MAX9880A (also 1.8V). The fallback (MCU bridge with NAU8822) remains documented but is now unlikely to be needed. The Waveshare HAT prototyping (NAU8810) validates the SIM7600 PCM voice output independent of this risk.
