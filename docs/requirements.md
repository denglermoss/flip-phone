...
- [x] **Codec selection (2026-06-28 — LOCKED, updated 2026-07-19)**: ~~**MAX9880A** selected as audio codec (Maxim/ADI, TQFN-48, ~$1.70).~~ **SUPERSEDED 2026-07-19**: MAX9880A replaced by **Realtek ALC5651-CG** (LCSC C963633, QFN-40 5×5mm, JLC Extended — no consignment). Same dual-port architecture: SIM7600 PCM → codec I2S-1 (voice), STM32H743 I2S → codec I2S-2 (music). **MCU is NOT in the voice audio path during calls.** Key findings: (1) all common codecs (WM8960, NAU8810, NAU8822, SGTL5000, TLV320AIC3204/3104, ES8316/8388) have only ONE digital audio port — cannot accept PCM and I2S simultaneously (the original "single codec unifies voice and music" claim was false); (2) the SIM7600 outputs PCM only (fixed master mode, short-frame sync, 16-bit linear — no I2S mode); (3) dual-port codecs are rare — MAX9880A (Maxim/ADI) and ALC5651/ALC5640/ALC5642 (Realtek) are the hobbyist-accessible options. ALC5651 selected over MAX9880A because it's JLC-sourced (eliminates Mouser consignment), has better specs (100dBA DAC vs 96dB, 94dBA ADC vs 82dB), smaller package (QFN-40 5×5mm vs TQFN-48 6×6mm), and has ASRC (asynchronous sample rate conversion per interface). PCM compatibility verified: ALC5651 §7.5.1 PCM Mode A (short sync) matches SIM7600 §3.6 (master, short sync, 16-bit, 2048/4096kHz). Fallback: MCU bridge with NAU8822 if ALC5651 unavailable. See project-log.md 2026-07-19 Codec Swap MAX9880A→ALC5651 and research-notes.md Codec Selection section.
...
- [x] **USB-C connector type (2026-07-19 — LOCKED)**: 16-pin USB-C (USB 2.0) for the main MCU USB connector. USB FS/HS only needs D+/D- — 24-pin USB 3.x is unnecessary. Micro-USB is obsolete and not considered. See project-log.md 2026-07-19.
- [x] **Modem USB HS port on rev1 (2026-07-19 — LOCKED)**: Route SIM7600 USB HS D+/D- to an **unpopulated connector footprint** on rev1. Preserves tethering + modem FW update + USB diagnostics + GNSS-over-USB + ecosystem options without a respin. MCU USB OTG_FS goes to the main populated USB-C. See project-log.md 2026-07-19.
- [x] **GNSS antenna on rev1 (2026-07-19 — LOCKED)**: Include **U.FL footprint** for GNSS antenna on rev1. SIM7600 has built-in GNSS (validated 2026-07-12). User wants this board to potentially be the final version — deferring would mean a respin. See project-log.md 2026-07-19.
- [x] **Loudspeaker on rev1 (2026-07-19 — LOCKED)**: Include **both earpiece and loudspeaker** on rev1. MAX9880A stereo outputs drive both at no extra codec cost. See project-log.md 2026-07-19.
- [x] **SIM + microSD connector strategy (2026-07-19 — DEFERRED TO SOURCING)**: Combo SIM+microSD socket vs separate sockets — decision deferred to BOM finalization based on reliable availability. If combo is reliably sourceable, use it (saves space); else use separate sockets. Both blocks appear in the block diagram. See project-log.md 2026-07-19.
